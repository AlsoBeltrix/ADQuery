...
internal sealed class ClaudeService : IClaudeService
{
    ...
    private string BuildExecutionPlanPrompt(string userQuery, string? context, int? requestedResultLimit)
    {
        ...
        promptBuilder.AppendLine("- Leave the subject ONLY when the user says so explicitly (\"forget Sanjay, everyone in China\", \"start over\", \"instead of...\"). Then drop the prior constraints and plan the new subject alone.");
