...
internal sealed class ClaudeService : IClaudeService
{
    ...
    private string BuildExecutionPlanPrompt(string userQuery, string? context)
    {
        ...
        promptBuilder.AppendLine("- Attribute names are accepted under EITHER the LDAP name or the PowerShell name — `l` and `City` both work, as do `st`/`State`, `c`/`Country`, `street`/`StreetAddress`, `physicalDeliveryOfficeName`/`Office`, `telephoneNumber`/`OfficePhone`, `sn`/`Surname`. Use whichever you prefer; a plan is never rejected for choosing one convention over the other. It IS rejected for naming an attribute that is not allow-listed at all, so prefer the common ones: displayName, sAMAccountName, mail, Title, Department, Manager, Company, Office/physicalDeliveryOfficeName, City/l, State/st, Country/c, employeeType, Enabled, AccountExpirationDate, MemberOf, extensionAttribute1-15, whenCreated, LastLogonDate.");
        ...
        promptBuilder.AppendLine("- A bare 'how many' question is a PURE COUNT: add the aggregation with count: true and an EMPTY group_by, and the scalar total is the answer. 'how many enabled users are there', 'how many contractors do we have', and 'how many people report to X' are all pure counts. Add group_by fields ONLY when the user asks for the breakdown ACROSS a value: 'how many users in each department', 'count by employee type', 'how many of those are contractors vs employees'. The test is whether the user asked for one number or a table of numbers.");
        promptBuilder.AppendLine("- NEVER group_by an attribute that a filter in the same plan has already pinned to a single value. Grouping `Enabled` inside a plan filtered to `Enabled` = \"true\" cannot produce a distribution — it yields one bucket plus whatever blank-valued rows the directory happens to hold, and turns a one-number answer into a table with a caveat about the grouping attribute. If a filter constrains the attribute, the count of it is the answer. This holds in follow-ups too, where the pinning filter came from an earlier turn.");
        promptBuilder.AppendLine();
        ...
        promptBuilder.AppendLine("- Worked example. Thread: \"everyone under Sanjay\" then \"only the ones with titles\" then \"add the users in China\". The third query means: people under Sanjay who have a title AND are in China — all three constraints at once. It does NOT mean every user in China across the directory — that would abandon the subject the user never left. It also does NOT mean Sanjay's titled reports plus a second, separate set of his reports in China: \"add\" brings a new constraint into the subject, it does not add a union arm.");
        promptBuilder.AppendLine("- Write the plan's \"description\" so it states the interpretation in full (\"Sanjay's reports with titles who work in China\"), so a misreading is visible to the user and correctable in one turn.");
        promptBuilder.AppendLine();
