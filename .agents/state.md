...
- **Entry point (2026-07-27) — new product direction.** Owner redirected the app toward answering the tedious, basic AD questions people bring to the team in person, rather than being a bulk-export tool. Captured as the first *feature* plan, `.agents/plans/F01-conversational-query.md` (the `F` namespace, distinct from the P01-P21 review-remediation plans). F01 has three threads, safest-first: (A) park CSV enrichment in the UI (server path stays intact), (B) derived headline answer keyed off plan shape, (C) byte-capped last-turn-only follow-up chat via the existing `context` parameter. **F01 status is `Draft` — not approved for implementation; no code lands until the owner flips its status line to `Approved`.** Supporting decisions recorded in `.agents/decisions.md`: DATA-D1, HEADLINE-D1, FOLLOWUP-D1, FOLLOWUP-D2, FONT-D1. Approved visual reference is the git-ignored mockup `artifacts/mockups/qa-ui.html` (design reference, not shipping code).
...
- **F01 awaits an owner go.** The plan is drafted and its decisions are recorded. Before implementation, confirm the three open items in F01's "Open items" section at implementation time (`FollowUp:MaxContextBytes` default, whether the sync `ExecuteQuery` path is still reachable, and the Slice A guard harness). Then implement safest-first: Slice A (park CSV in UI), Slice B (headline), Slice C (follow-up chat), each a separate commit with a red→green guard and `scripts/verify.ps1`.
