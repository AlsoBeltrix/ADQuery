...
| slice-c1 | MEDIUM  | Mis-sized/unenforced follow-up cap → over-limit context persisted/logged/transmitted, or corrupt fragment | `[~]`  | —      | codex/@azure-openai-eus2-global/gpt-5.5-dzs/xhigh/standard (`--profile review`, owner-authorized) — R1 **reopened** (retry enqueue path unenforced + chained directives), repaired `8549ecc`, R2 pending redispatch (frontier fail-closed → owner) |
