...
## Known gaps
Whether `Failed` is reachable after `:403` was not established while writing this record; the
fix should cover any non-`Completed` terminal transition rather than special-casing
`Cancelled`, which makes the question moot for correctness, but the guard should state which
transitions it actually exercises.
