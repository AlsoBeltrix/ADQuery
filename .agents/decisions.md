...
## F06-D1 — Merging adquery into ExchangeAdminWeb is deferred; Exchange support in adquery is not

- Status: Approved
- Date: 2026-08-03
- Authority: Repository owner: *"no. defer integration. this app needs to stand on its own properly first."* — then, correcting the agent's over-broad reading: *"what's deferred is integrating this with exchangeadminweb."*
- Decision: **What is deferred is the merge with ExchangeAdminWeb, not Exchange Online as a data source.** Specifically deferred: porting adquery into EAW, rebuilding its UI as Razor, extracting the LLM into a shared host service, and riding EAW's connection pool or app registration. adquery remains a standalone application and must be sound as one.
- **Explicitly NOT deferred: F06 Slices 3-4, Exchange Online read-only room support inside adquery.** The determination stands on measured evidence — AD holds 147 room mailboxes to Exchange's 1,931, and 0 Chelmsford rooms to Exchange's 16 — and the capability is adquery's to build for itself. What changes is *how it gets a credential*: not by borrowing EAW's, which was the reason F06-Q1 looked hard.
- Question status, corrected: **F06-Q2** (roll into EAW) is **answered: no**. **F06-Q3** (port-in vs extract-out) is **moot** — it only existed inside the merge. **F06-Q1** (how adquery authenticates to Exchange) is **reopened and is the live blocker for Slice 3**; the owner's read-only-secret instruction stands as its intended shape. **F06-Q4** (tenant rights for a read-only EXO app registration) is **live**, since adquery now needs its own rather than EAW's.
- Consequence: `.agents/plans/F07-read-only-assist-boundary.md` is **Deferred** — its three-layer boundary was designed for an LLM sitting inside a write-privileged host, a situation that no longer arises. Its analysis is kept intact rather than deleted; the read-only-credential reasoning still applies to F06-Q1, and the call-graph guard is a technique worth reusing if adquery ever gains write capability. F06 Slices 1-2 are landed and were never Exchange work.
- Rationale for the standalone half: adquery has four fixes committed but undeployed, and two of its last three defect classes were found by the owner using it rather than by 392 passing tests. Adding a second data source to an application in that state repeats the pattern. "Stands on its own properly" is a quality bar on the existing app, and it gates *how much new work runs in parallel*, not whether Exchange is ever supported.
...
- Constraints (**pending implementation** — Slice 6 has not landed; this states what it must satisfy): Lands with red→green guards: a thread longer than the max-prior-questions bound drops oldest-first and never drops the current turn's question — proven to fail with the bound removed; a thread of `MaxPriorQuestions` **maximum-length** questions plus maximum fixed components composes within the backstop with no component dropped — proven to fail at a 32 KB / 50-question pair; the knob pair is startup-validated and a pair violating the derived floor fails at boot — proven to fail if the validation is removed; an over-cap composition raises a logged error rather than a truncated context — proven to fail against the current drop-ladder; the assembled context carries **no** accumulated result rows or value slices beyond the last-turn DATA-D1 bound — proven to fail if results accumulate; and the transport attribute, constant, and validator ceiling remain reconciled. Must pass `scripts/verify.ps1` before the slice is claimed complete.
- Consequence: A long thread stays answerable instead of silently forgetting its own early constraints, at the cost of a larger prompt per turn. DATA-D1's "minimal slice" is enforced where it is meaningful — the row and bucket caps — rather than by a byte ceiling that was never sized against anything. To be implemented by F04 Slice 6; `.agents/plans/F04-genuine-conversational-answers.md` D6 is ruled, and all six F04 owner decisions are now settled. Whether compaction summarizes dropped questions (a model call) or simply drops them is an implementation-time choice; dropping is the floor. Open at implementation time: whether Narrate's input is bounded by this same knob or its own — it is a separate call with a different payload shape, and the plan says only "under a byte cap."
...
- Constraints (**pending implementation** — Slice 7 has not landed; this states what it must satisfy): Lands with red→green guards: a freshly completed job with the cache absent or evicted still serves preview, a single-record headline that stays `record` rather than degrading to `count`, download, and cross-turn reuse — proven to fail when any single reader is left on the cache; and a large (e.g. 40k-row) result exports from the artifact with no full-set in-RAM copy. Must pass `scripts/verify.ps1` before the slice is claimed complete.
- Consequence: A 40k-row thread no longer pins server memory, and export stops depending on a cache entry that can expire. To be implemented by F04 Slice 7, sequenced last so the conversational path is proven before the storage model changes underneath it; `.agents/plans/F04-genuine-conversational-answers.md` D5 is ruled. **Artifact lifecycle is in scope for Slice 7 (amended by f04-or-7):** this decision originally deferred retention and cleanup on `E:\WWWOutput` as an open implementation-time question, which is unsafe — dropping the 2h cache removes the only expiry bounding result residency, and `CleanupCompletedJobs` (`QueryJobManager.cs:454-466`) deletes in-memory metadata only, so artifacts would accumulate permanently and orphan themselves once their job expires. Slice 7 must delete artifacts with job expiry (file before metadata), sweep crash orphans at startup, admit queries against free disk space, and define ownership for artifacts reused across turns.
...
- Constraints (**pending implementation** — Slice 1b has not landed; this states what it must satisfy): One concern, landing with a red→green guard: grouping `["Contractor","contractor","CONTRACTOR", blank]` yields one `Contractor` bucket of 3 with a stable display key plus one `(empty)` bucket, and reports 3 spellings; the same input under the case-sensitive flag yields three buckets. Proven to fail against the case-sensitive keying. Must pass `scripts/verify.ps1` before the slice is claimed complete.
- Consequence: "Most common value" reflects human intent by default, exact-case analysis stays reachable by asking for it, and case inconsistency surfaces without the user knowing the feature exists. To be implemented by F04 Slice 1b; `.agents/plans/F04-genuine-conversational-answers.md` D4 is ruled. The singleton/blank/distinct counts F04-OR-3 requires for Narrate are computed over the case-folded buckets.
...
- Constraints (**pending implementation** — Slice 5 has not landed; this states what it must satisfy): One concern, landing with a red→green guard covering **all four** negation operators (`not_equals`, `not_contains`, `not_starts_with`, `not_ends_with`) with an empty value: each returns the populated subset rather than `Projection filter value is required` or the empty set, across missing / blank / whitespace-only / populated / multivalued attributes. Proven to fail against the current strict path. Must pass `scripts/verify.ps1` before the slice is claimed complete. **Correction (f04-or-5):** executor semantics are not uniform across the four — `MatchesBaseOperator` (`DirectoryPlanExecutor.cs:1486-1488`) short-circuits `contains`/`starts_with`/`ends_with` to true on an empty needle, so under negation those three match nothing. Implement via one explicit populated-attribute predicate ahead of operator dispatch; a guard covering only `not_equals` is vacuous.
- Consequence: The most natural way for a model to express "has a value" stops being a hard error. To be implemented by F04 Slice 5, sequenced early because it is a self-contained bug fix independent of the conversational rework; `.agents/plans/F04-genuine-conversational-answers.md` D3 is ruled.
...
- Constraints (**pending implementation** — Slice 1 has not landed; this states what it must satisfy): One concern, landing with a red→green guard. The guard feeds an extensionAttribute1-shaped plan and result and asserts the settled result retains its grouped aggregation and does not expand to one row per distinct value, and that the export of a grouped result is the value+count table. Proven to fail against the pre-removal tree. Must pass `scripts/verify.ps1` before the slice is claimed complete.
- Consequence: Unique-list, distinct-values, and most-common questions return grouped counts and download as a distribution table. To be implemented by F04 Slice 1 (with the export half landing alongside Slice 4's format work); `.agents/plans/F04-genuine-conversational-answers.md` D2 is ruled. `HeadlineClassifier`'s step-3 comment referencing the transform goes stale and is updated with the removal.
...
- Consequence: One extra call's latency and tokens per turn, accepted by the owner. To be implemented by F04 Slice 2; `.agents/plans/F04-genuine-conversational-answers.md` D1 is ruled.
...
- Consequence: The approved mockup (`artifacts/mockups/qa-ui.html`) already satisfies this; porting its palette/typography into `csharp/wwwroot/css/styles.css` needs no font-delivery infrastructure.
