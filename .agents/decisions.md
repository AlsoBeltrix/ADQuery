...
- Decision: Three parts.
  ...
  2. **The budget is sized to the job, not to a small fixed cap.** The context sent is what the turn needs, bounded by a **context budget** derived from the route's model context window, with **compaction before that window is reached** rather than a hard truncation at an arbitrary small number. Compaction drops or summarizes **oldest questions first**; the current turn's question and the last turn's plan summary/value slice are never dropped. This replaces the fixed-2000-byte behavior, not the existence of a bound.
  3. **A conservative default where the window is not reliably known.** The per-route context window is not discoverable from configuration today, and the two MODEL-D1 routes differ. Where the window for the effective route is not reliably known, a conservative default budget applies. `LlmProviderOptions.MaxTokens` (`csharp/Configuration/LlmProviderOptions.cs:21`, `"4000"`) is the **output** cap consumed at `ClaudeService.cs:113` — it is not a context window and must not be used to derive this budget.
- Scope: Implementing this **requires deliberately widening the transport guard**, which currently forbids it. `FollowUpOptions.ContextTransportCodeUnitLimit` is `2000` and `MaxContextBytes` defaults to it (`csharp/Configuration/FollowUpOptions.cs`); `FollowUpOptionsValidator.cs:20-28` hard-rejects any configured value above that limit with a message stating a larger context "needs a deliberate widening of the `QueryRequest.Context` `[StringLength]` attribute." That `[StringLength(2000)]` on `QueryRequest.Context`, the constant, the validator ceiling, and `FollowUpOptionsReconciliationTests` (which asserts they agree) move **together in one commit** — this decision is that deliberate widening. The advisory client-side copy at `QueryController.cs:243-250` follows the same value; the server bound stays authoritative (FOLLOWUP-D1, DATA-D1).
...
- Constraints: Landed with red→green guards: a thread whose accumulated questions exceed the budget compacts oldest-first and stays within it, never dropping the current question or the last-turn slice — proven to fail with compaction disabled; the assembled context carries **no** accumulated result rows or value slices beyond the last-turn DATA-D1 bound — proven to fail if results accumulate; and the transport attribute, constant, and validator ceiling remain reconciled. Passes `scripts/verify.ps1`.
- Consequence: A long thread stays answerable instead of silently forgetting its own early constraints, at the cost of a larger prompt per turn and a transport limit that is no longer a small fixed number. Implemented by F04 Slice 6; `.agents/plans/F04-genuine-conversational-answers.md` D6 is ruled, and all six F04 owner decisions are now settled. Whether compaction summarizes dropped questions (a model call) or simply drops them is an implementation-time choice within this budget; dropping is the floor.
