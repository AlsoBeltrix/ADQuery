...
- **Concurrent active-CSV admission count: DEFERRED** — owner decision 2026-07-23. This is the only D1 cap that cannot be responsibly closed from Slice 0 evidence, because it has two external dependencies neither of which Slice 0 produces: (1) a **server-GC memory measurement** to derive the safe concurrent count — the deployed app defaults to server GC (Web SDK, no override in `csharp/AdQueryOrchestrator.csproj`), but every heap/working-set figure here was measured under workstation GC; and (2) **live AD per-job timing** (the deferred P09 live-timing matrix) to derive per-job duration. The owner established a binding precondition: serializing to one concurrent job is acceptable ONLY if queued users can be shown a reasonable wait time, and a reasonable wait time requires the per-job duration that only live AD timing provides. Therefore no interim count (neither serialize-to-1 nor a guessed small number) is adopted. The count is set later from those two measurements, with fresh owner approval. GC caveat and P09 timing dependency are recorded below in Caveats. (Defers choice 5.)
