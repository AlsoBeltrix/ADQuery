...
4. Publishes the application framework-dependently to a unique temporary directory using the P03-approved SDK/target and the checked native runner.
5. Requires the primary application DLL, `.deps.json`, `.runtimeconfig.json`, `web.config`, static assets, and configuration allow-list/prompt files expected by the project.
...
- Add tamper, extra/missing file, duplicate/case collision, zip-slip, oversized archive, same-ID mismatch, and concurrent-lock tests.
- Do not invoke IIS.

Guard proof: bypass one extracted-file hash comparison; the tampered-content test must fail. Restore and verify green.
...
6. Package hash mismatch, malformed manifest, duplicate JSON key, unknown field, invalid release ID, and wrong source commit fail.
...
- Packaging requires clean committed source, canonical verification, checked publish, closed manifest, safe archive, and explicit expected package hash.
